...
const selfstarWarningTimeout = process.env.HOSHI_SELFSTAR_WARNING_TIMEOUT;
...
class Starboard {
  ...
  get channel() {
    const channelID =
        this.client.settings.get(this.guild, 'starboardChannelID');
  ...
  add(message, starredBy) {
    ...
    if (!this.channel) {
      ...
      return `There isn't a starboard channel to use. Set one using the \`${
          prefix}starboard\` command!`;
    }
    ...
    if (message.author.id === starredBy.id) {
      if (!this.recentlySelfstarred.has(message.author.id)) {
        ...
        setTimeout(
            () => this.recentlySelfstarred.delete(message.author.id),
            selfstarWarningTimeout);
  ...
  async removeStar(message, unstarredBy) {
    ...
    if (message.reactions.has(emojiID)) {
      ...
      if (reaction.users.has(unstarredBy.id)) {
        await reaction.users.remove(unstarredBy)
            .then(() => {
    ...
    if (newStarredBy.length) {
      ...
      if (newStarredBy.length >= this.threshold) {
        ...
        const msg = await this.channel.messages.fetch(star.starboardMessageID)
                        .catch(() => null);
  ...
  async fixStar(message) {
    ...
    if (!star) {
      ...
      const starredBy = users.map(user => user.id)
                            .filter(
                                user => message.author.id !== user &&
    ...
    const newStarredBy =
        users.map(user => user.id)
            .filter(
                user => !star.starredBy.includes(user) &&
                    message.author.id !== user && !blacklist.includes(user))
  ...
  missingPermissions() {
    const {missingPermissions} =
        this.client.listenerHandler.modules.get('missingPermissions');
    const str = missingPermissions(this.channel, this.client.user, [
      'VIEW_CHANNEL', 'MANAGE_MESSAGES', 'READ_MESSAGE_HISTORY',
  ...
  static findAttachment(message) {
    ...
    const richEmbed = message.embeds.find(
        embed => embed.type === 'rich' && embed.image &&
    ...
    const attachment = message.attachments.find(
        file => extensions.includes(path.extname(file.url)));
    if (attachment) {
